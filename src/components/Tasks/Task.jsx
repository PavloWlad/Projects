...
const Task = ({ id, text, completed, event, onRemove, onEdit, onComplete }) => {
  const onChangeCheckbox = e => {
    onComplete(event.id, id, e.target.checked);
  };
    ...
    <div key={id} className="tasks__items-row">
      <div className="checkbox">
        <input
          ...
          checked={completed}
        />
        <label style={{borderColor: event.color.hex, backgroundColor: 'white' }} htmlFor={`task-${id}`}>
          <svg
            width="11"
            height="8"
            viewBox="0 0 11 8"
          ...
          >
            <path style={{stroke: event.color.hex }}
      ...
      <div className="tasks__items-row-actions">
        <div onClick={() => onEdit(event.id, { id, text })}>
          <svg
            width="15"
            height="15"
            viewBox="0 0 15 15"
        ...
        <div onClick={() => onRemove(event.id, id)}>
          <svg
            width="11"
            height="11"
            viewBox="0 0 11 11"
